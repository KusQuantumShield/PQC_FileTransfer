...
1. **의존성 라이브러리 및 패키지 설치:**
    프로젝트 폴더에서 다음 명령어를 통해 패키지를 설치합니다. 최신 리눅스(Kali, Ubuntu 등) 환경에서는 의존성 충돌을 피하기 위해 `pipx`를 사용하여 전역 명령어로 설치하는 것을 권장합니다.

    ```bash
    # (권장) pipx를 통한 프로젝트 및 전역 명령어(pqc-server 등) 설치
    pipx install -e .

    # (대안) 일반 가상환경 또는 시스템 파이썬(권장하지 않음) 환경 시
...
2. **패키지 설치:** TUI 실행을 위한 `windows-curses` 패키지가 의존성에 기본 포함되어 있어, 별도 설치 없이 다음 명령어로 한 번에 설치할 수 있습니다.
