...
1. **의존성 라이브러리 및 패키지 설치:**
    가장 빠르고 의존성 충돌 없는 환경 구성을 위해 **`uv`** 사용을 적극 권장합니다.
    프로젝트 폴더에서 다음 명령어를 통해 가상환경 생성 및 패키지(테스트/개발 포함)를 설치합니다.

    ```bash
    # 1. 가상환경 생성 (.venv 디렉토리가 생성됨)
    uv venv

    # 2. 가상환경 활성화
    source .venv/bin/activate

    # 3. 프로젝트 및 의존성 설치 (테스트/개발용 패키지 포함)
    uv pip install -e ".[dev]"
    # (참고: uv sync --all-extras 명령어를 사용하셔도 됩니다.)
    ```

    *참고: `pipx`나 일반 `pip`를 사용하실 수도 있습니다.*
...
2. **패키지 설치 (uv 권장):** TUI 실행을 위한 `windows-curses` 패키지가 의존성에 기본 포함되어 있어, 별도 설치 없이 다음 명령어로 한 번에 설치할 수 있습니다.
...
(`uv` 환경을 구축한 경우, 가상환경을 활성화하거나 명령어 앞에 `uv run`을 붙여서 실행합니다. 예: `uv run pqc-tui`)
