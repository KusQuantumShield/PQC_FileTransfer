...
def recv_exact(sock: socket.socket, length: int) -> bytes:
    ...
    view = memoryview(buf)
    recv_exact_into(sock, view, length)
    return bytes(buf)
...
def configure_socket(sock: socket.socket, is_server: bool = False) -> None:
    """
    유지보수성을 위해 공통 소켓 설정(버퍼 크기, 재사용, Nagle 알고리즘 비활성화)을 캡슐화합니다.
    """
    if is_server:
