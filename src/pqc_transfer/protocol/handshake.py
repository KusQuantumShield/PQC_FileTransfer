...
def perform_client_handshake(sock: socket.socket, server_ip: str) -> bytes:
    """
    ...
    with oqs.KeyEncapsulation(config.KEM_ALG) as kem:
        ...
        if not key_manager.verify_and_trust_server(server_ip, server_sig_pk):
...
def perform_server_handshake(conn: socket.socket) -> bytes:
    """
    서버 관점의 KEM 핸드셰이크 수행
    """
    kem_start_time = time.perf_counter()
    ...
    sig_public_key, sig_secret_key = key_manager.get_server_sig_keys()
    with oqs.Signature(config.SIG_ALG, secret_key=sig_secret_key) as signer:
    ...
    with oqs.KeyEncapsulation(config.KEM_ALG, secret_key=secret_key) as kem:
