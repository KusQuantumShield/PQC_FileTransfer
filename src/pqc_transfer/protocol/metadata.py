...
def send_metadata(
    conn: network.SecureConnection, client_id: str, filename: str, filesize: int
) -> None:
    """
...
def receive_metadata(conn: network.SecureConnection) -> tuple[str, str, int]:
    ...
    """
    client_id = conn.recv_with_length(max_len=constants.MAX_CLIENT_ID_LEN).decode(
        "utf-8"
    )
    ...
    logger.log(
        "INFO",
        "FILE",
        f"수신 파일 정보 - 이름: {filename}, 크기: {filesize} 바이트, 클라이언트 ID: {client_id}",
    )
