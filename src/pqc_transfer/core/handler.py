...
class PQCServerHandler:
    ...
    def handle(self) -> bool:
        ...
        try:
            ...
            temp_path, received_size = self.receive_file_chunks(session_key, original_filesize)
            self.verify_signature(client_id, filename, original_filesize, received_size, session_key)
    ...
    def perform_handshake(self) -> bytes:
        ...
        """
        try:
            return handshake.perform_server_handshake(self.conn, self.config.kem_alg, self.config.sig_alg, self.key_manager)
        except Exception as e:
    ...
    def receive_file_chunks(self, session_key: bytes, original_filesize: int) -> tuple[str, int]:
        """
        ...
        """
        try:
            receiver = chunk_receiver.ChunkReceiver(self.conn, session_key, self.file_hasher, self.config.chunk_size)
            temp_path, received_size = receiver.receive(original_filesize, self.config.save_dir)
            self.temp_path = temp_path
            return temp_path, received_size
        except exceptions.PQCBaseError as e:
            self.abort(str(e))

    def verify_signature(self, client_id: str, filename: str, original_filesize: int, received_size: int, session_key: bytes) -> None:
        """
