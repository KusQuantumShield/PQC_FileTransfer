...
class PQCClient:
    ...
    def __init__(self, file_path: str, app_config: AppConfig, key_manager: KeyManager) -> None:
        """
    ...
    def transfer(self) -> None:
        ...
        try:
            ...
            logger.log("ERROR", "CLIENT", f"서버 {self.config.server_ip}:{self.config.port} 에 연결할 수 없습니다. 서버가 실행 중인지 확인하세요.")
    ...
    def transfer_file_chunks(self, conn: network.SecureConnection, session_key: bytes) -> tuple[int, str]:
        ...
        sender = chunk_sender.ChunkSender(conn, session_key, file_hasher, self.config.chunk_size)
