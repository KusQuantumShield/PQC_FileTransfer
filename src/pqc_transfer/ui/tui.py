...
class TUIApp:
    ...
    def run_subprocess_ui(self, cmd_args, title):
        ...
        Args:
            cmd_args (list): 실행할 명령어 및 인자 리스트 (예: [sys.executable, "-m", "pqc_transfer.core.server"])
        ...
        # 모듈 방식 실행(-m)을 위해 src 디렉토리를 PYTHONPATH에 추가
        ...
        while True:
            ...
            # 엔터키: 현재 항목 실행
                ...
                if self.current_row == 0:
                    # 1. Start PQC Server 선택: 서브프로세스로 서버 실행
                    self.run_subprocess_ui([sys.executable, "-m", "pqc_transfer", "server"], "PQC Server Logs")
                elif self.current_row == 1:
                    # 2. Start PQC Client 선택: 전송할 파일 선택 후 클라이언트 실행
                    file_to_send = self.file_picker()
                    if file_to_send:
                        # 선택된 파일 경로를 인자로 전달
                        self.run_subprocess_ui([sys.executable, "-m", "pqc_transfer", "client", file_to_send], "PQC Client Logs")
